...
const FLAGS = {
  ...
  kansas_submission_gateway_enabled: true,
};

const BUILD_VERSION = "CG Gateway Build v0.1 — Kansas";
...
// ── Synthetic Brian Mitchell case ────────────────────
const BRIAN_CASE = {
  id: "CASE-KS-2026-5701",
  patient: "Brian Mitchell",
  mrn: "MRN560201",
  dob: "1965-04-15",
  program: "KanCare",
  programScope: "KanCare — annual renewal",
  state: "Kansas",
  zip: "67501",
  renewalDue: "2026-11-30",
  daysRemaining: 42,
  created: "2026-09-03T09:14:00",
  humanOwner: "Navigator — S. Dunn",
  ...
  stateProfile: {
    ...
    portalUrl: "kancare.ks.gov",
    portalFull: "https://www.kancare.ks.gov",
    authRepMethod: "Electronic designation / form",
    ldssRouting: "ZIP 67501 → KDHE KanCare county office — Reno County",
    ...
    kffSource: "KDHE official source + KMAP enrollment data",
    automationPolicy: "Human-assisted only",
    notes: "Kansas navigator ecosystem. KMAP is the state eligibility verification system. Ex parte review required before paper request. Consumer third-party account access confirmed. No public API. Institutional submission pathway requires state verification.",
  },

  authority: {
    status: "verified",
    type: "KanCare certified navigator",
    beneficiaryAuth: "On file — CG-CA-01",
    beneficiaryAuthDate: "2026-09-01",
    orgCredential: "Prairie Star Health FQHC — credentialed",
    submissionPermitted: true,
    blockingReason: null,
    evidence: "CG-CA-01_KSDHS_signed_20260901.pdf",
  },

  evidence: [
    { id: "ev1", label: "Medicaid renewal notice",            status: "verified",   source: "State notice",  file: "Renewal_notice_Mitchell.pdf",       required: true,  sensitive: false },
    ...
    { id: "ev4", label: "Income verification",   status: "verified",   source: "Staff upload",  file: "Income_verification_Mitchell.pdf",          required: true,  sensitive: false },
  ...
  route: {
    ...
    why: "Navigator holds certified assister credential and beneficiary authorization is verified. No public API available for Kansas. Consumer portal handoff also available as fallback.",
    automationAllowed: false,
    humanRequired: true,
    fallback: "L1 — Consumer portal handoff",
    destination: "KanCare / KDHE — assister account",
    ldssRouting: "ZIP 67501 → KDHE KanCare county office — Reno County",
  ...
  stateStatus: {
    ...
    canonical: null,
    profileVersion: "v2026.09",
    illustrativeNote: "KDHE portal status strings not yet confirmed from official source. Raw state response will appear here once submission is made and state portal responds.",
  },

  coverageState: "PENDING_RECONCILIATION",

  auditEvents: [
    { time: "09:14", actor: "System",          type: "complete", label: "State profile resolved",               detail: "KanCare / KDHE v2026.09 · confidence Moderate" },
    { time: "09:15", actor: "Navigator",        type: "complete", label: "Authority validated",                  detail: "Certified assister · CG-CA-01 on file · Prairie Star Health FQHC credentialed" },
    ...
    { time: "09:22", actor: "System",          type: "complete", label: "Route selected · L2 assister portal",  detail: "No public API · authority verified · LDSS ZIP 67501" },
    { time: "09:23", actor: "System",          type: "action",   label: "Submission handoff pending",            detail: "Awaiting navigator action in KanCare / KDHE" },
  ],
};

// ── Patient 2 — Tamika Reynolds · Wichita · KDHE online ──
const TAMIKA_CASE = {
  id: "CASE-KS-2026-5702",
  patient: "Tamika Reynolds",
  mrn: "MRN560302",
  dob: "1990-02-14",
  program: "KanCare",
  programScope: "KanCare — annual renewal",
  state: "Kansas",
  zip: "67214",
  renewalDue: "2026-12-01",
  daysRemaining: 89,
  created: "2026-09-03T10:00:00",
  humanOwner: "Navigator — L. Ortiz",
  stateProfile: {
    name: "KDHE Online Portal",
    version: "v2026.09",
    tier: "L1",
    tierLabel: "Consumer portal — KanCare self-service",
    portalUrl: "kancare.ks.gov",
    portalFull: "https://www.kancare.ks.gov",
    authRepMethod: "Consumer self-service with navigator guidance",
    ldssRouting: "ZIP 67214 → Sedgwick County DCF",
    supportsOnlineRenewal: true, supportsDocUpload: true, supportsThirdParty: true, supportsPublicApi: false, cmsEmmyPartner: false,
    confidence: "Moderate",
    sourceUrl: "https://www.kancare.ks.gov/",
    sourceChecked: "2026-09-03",
    kffSource: "KDHE + official state source",
    automationPolicy: "Human-assisted only",
    notes: "Wichita metro. Consumer portal pathway. Navigator guides patient through KanCare self-service renewal.",
  },
  authority: { status: "verified", type: "Consumer self-service (navigator-guided)", beneficiaryAuth: "On file — CG-CA-01", beneficiaryAuthDate: "2026-08-18", orgCredential: "Wichita FQHC — credentialed", submissionPermitted: true, blockingReason: null, evidence: "CG-CA-01_signed_20260818.pdf" },
  evidence: [
    { id: "ev1", label: "KanCare renewal notice",              status: "verified",  source: "State notice",  file: "Renewal_notice_Reynolds.pdf",   required: true, sensitive: false },
    { id: "ev2", label: "Identity — government-issued ID",   status: "verified",  source: "EMR — eCW",     file: "ID_verified_EMR.pdf",            required: true, sensitive: false },
    { id: "ev3", label: "Authorized rep consent — CG-CA-01", status: "verified",  source: "CG signed",     file: "CG-CA-01_consent_signed.pdf",    required: true, sensitive: false },
    { id: "ev4", label: "Income verification — paystubs",    status: "verified",  source: "Staff upload",  file: "Paystub_Reynolds.pdf",           required: true, sensitive: false },
    { id: "ev5", label: "Address verification",               status: "verified",  source: "Staff upload",  file: "Utility_bill_Reynolds.pdf",      required: true, sensitive: false },
  ],
  packageVersion: 1, packageItems: ["ev1","ev2","ev3","ev4","ev5"],
  route: { tier: "L1", label: "L1 — Consumer portal handoff", why: "Patient has KanCare account. Navigator guides through self-service.", automationAllowed: false, humanRequired: true, fallback: "L2 — Assister account", destination: "KanCare portal — consumer account", ldssRouting: "ZIP 67214 → Sedgwick County DCF", profileVersion: "v2026.09", credentialsRequired: ["Patient KanCare login", "Navigator present"] },
  canonicalStatus: "READY_TO_SUBMIT",
  stateStatus: { raw: null, canonical: null, profileVersion: "v2026.09", illustrativeNote: "Awaiting consumer portal submission." },
  coverageState: "PENDING_RECONCILIATION",
  auditEvents: [
    { time: "10:00", actor: "System",    type: "complete", label: "State profile resolved",   detail: "KDHE Online Portal v2026.09 · L1 consumer portal" },
    ...
    { time: "10:06", actor: "System",    type: "action",   label: "Submission handoff pending", detail: "Awaiting patient login to KanCare portal" },
  ],
};

// ── Patient 3 — David Nguyen · Rural · Paper mail ──
const DAVID_CASE = {
  id: "CASE-KS-2026-5703",
  ...
  state: "Kansas",
  zip: "67801",
  renewalDue: "2026-10-15",
  daysRemaining: 41,
  created: "2026-09-03T10:30:00",
  humanOwner: "Navigator — S. Brown",
  stateProfile: {
    name: "Finney County DCF",
    version: "v2026.09",
    tier: "L0",
    tierLabel: "Manual — paper mail to county DCF",
    portalUrl: "dcf.ks.gov",
    portalFull: "https://www.dcf.ks.gov",
    authRepMethod: "Form filed with county DCF",
    ldssRouting: "ZIP 67801 → Finney County DCF",
    ...
    confidence: "Low",
    sourceUrl: "https://www.dcf.ks.gov/",
    sourceChecked: "2026-09-03",
    kffSource: "KDHE + official state source",
    automationPolicy: "Manual only",
    notes: "Rural western Kansas. No confirmed electronic portal. Paper renewal packet required.",
  },
  authority: { status: "verified", type: "Certified assister / navigator", beneficiaryAuth: "On file — CG-CA-01", beneficiaryAuthDate: "2026-08-22", orgCredential: "Garden City FQHC — credentialed", submissionPermitted: true, blockingReason: null, evidence: "CG-CA-01_signed_20260822.pdf" },
  evidence: [
    { id: "ev1", label: "KanCare renewal notice",              status: "verified",  source: "State notice",  file: "Renewal_notice_Nguyen.pdf",     required: true, sensitive: false },
    { id: "ev2", label: "Identity — government-issued ID",   status: "verified",  source: "EMR — eCW",     file: "ID_verified_EMR.pdf",            required: true, sensitive: false },
    { id: "ev3", label: "Authorized rep consent — CG-CA-01", status: "verified",  source: "CG signed",     file: "CG-CA-01_consent_signed.pdf",    required: true, sensitive: false },
    { id: "ev4", label: "Income verification — tax return",  status: "present",   source: "Staff upload",  file: "Tax_return_Nguyen.pdf",          required: true, sensitive: false },
    { id: "ev5", label: "Address verification",               status: "missing",   source: null,            file: null,                             required: true, sensitive: false,
      missingNote: "Patient relocated from Garden City. New address documentation pending." },
  ],
  packageVersion: 1, packageItems: ["ev1","ev2","ev3","ev4"],
  route: { tier: "L0", label: "L0 — Paper mail to DCF", why: "Finney County DCF has no confirmed electronic portal. Paper packet required.", automationAllowed: false, humanRequired: true, fallback: "None — paper only", destination: "Finney County DCF — physical mail", ldssRouting: "ZIP 67801 → Finney County DCF", profileVersion: "v2026.09", credentialsRequired: ["Staff attestation", "DCF receipt if provided"] },
  canonicalStatus: "READY_TO_SUBMIT",
  stateStatus: { raw: null, canonical: null, profileVersion: "v2026.09", illustrativeNote: "Paper submission — no electronic tracking." },
  coverageState: "PENDING_RECONCILIATION",
  auditEvents: [
    { time: "10:30", actor: "System",    type: "complete", label: "State profile resolved",   detail: "Finney County DCF v2026.09 · L0 paper mail" },
    ...
    { time: "10:36", actor: "System",    type: "action",   label: "Manual submission pending",  detail: "Navigator must mail packet to Finney County DCF" },
  ],
};

const ALL_KS_CASES = [
  { ...BRIAN_CASE,   label: "Brian Mitchell",    tag: "Reno · MRN560201",    tagColor: "#8B6914" },
  { ...TAMIKA_CASE,  label: "Tamika Reynolds",   tag: "Wichita · MRN560302", tagColor: "#4B3C96" },
  { ...DAVID_CASE,   label: "David Nguyen",      tag: "Finney · MRN560403",  tagColor: "#D97706" },
];
    ...
    <Card>
      ...
      <div style={{ padding: "12px 14px" }}>
        ...
        <div style={{ background: T.bgGreen, border: `1px solid ${T.green}33`, borderRadius: 8, padding: "8px 12px", fontSize: 12, color: T.green, display: "flex", gap: 6, alignItems: "flex-start" }}>
          ...
          <span>Authority verified. Navigator may proceed as authorized assister on behalf of Brian Mitchell through KanCare / KDHE. Credential sharing is not permitted — assister must log in with their own organization account.</span>
        </div>
      </div>
    </Card>
  );
    ...
    <Card style={{ border: `2px solid ${T.primary}` }}>
      ...
      <div style={{ padding: "14px" }}>
        <div style={{ background: T.bgAmber, border: `1px solid ${T.amber}40`, borderRadius: 8, padding: "9px 12px", marginBottom: 12, fontSize: 12, color: T.amber, display: "flex", gap: 7, alignItems: "flex-start" }}>
          ...
          <span>Do not share Brian Mitchell' credentials. Log in to KanCare / KDHE using your own certified assister organization account. CoverageGuard does not and cannot access the portal on your behalf.</span>
        </div>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          <a href="https://www.kancare.ks.gov" target="_blank" rel="noopener noreferrer" style={{ padding: "7px 13px", borderRadius: 8, border: "none", background: T.primary, fontSize: 12, fontWeight: 700, color: "#fff", cursor: "pointer", display: "flex", gap: 5, alignItems: "center", textDecoration: "none" }}>
            <ExternalLink size={13}/> Open KanCare / KDHE
          </a>
          <input
            value={receipt}
    ...
    <Card>
      ...
      <div style={{ padding: "12px 14px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ background: T.canvas, borderRadius: 9, padding: "10px 12px" }}>
            <div style={{ fontSize: 10.5, color: T.muted, marginBottom: 4, fontWeight: 700 }}>KDHE state status</div>
        ...
        {expanded && (
          <div style={{ marginTop: 10, background: T.light, border: `1px solid ${T.primary}22`, borderRadius: 9, padding: "11px 13px", fontSize: 12, color: T.charcoal, lineHeight: 1.7 }}>
            <div style={{ fontWeight: 700, color: T.navy, marginBottom: 6 }}>Status normalization — KanCare / KDHE</div>
            ...
            <div style={{ marginBottom: 4 }}>Mapped using: Kansas State Profile {c.stateProfile.version}</div>
            <div style={{ marginTop: 8, padding: "7px 10px", background: T.bgAmber, borderRadius: 7, color: T.amber, fontSize: 11.5, lineHeight: 1.5 }}>
              ⚠ Illustrative note: KanCare / KDHE portal status strings (e.g. "Pending Review", "Document Not Accepted") have not yet been confirmed from an official state source. Status mapping will be populated once KDHE returns a documented response. CoverageGuard will never infer acceptance from silence.
            </div>
            <div style={{ marginTop: 8, fontSize: 11, color: T.muted }}>Once KDHE responds, this panel will show: raw status → canonical status → mapping rule used → profile version → timestamp.</div>
          </div>
        )}
      </div>
    </Card>
  ...
  return (
    <div style={{ width: 180, background: T.navy, flexShrink: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "14px 14px 12px", borderBottom: `1px solid rgba(255,255,255,.08)` }}>
        ...
        <div style={{ fontSize: 9.5, color: "rgba(255,255,255,.3)", marginTop: 2, textTransform: "uppercase", letterSpacing: .5 }}>KANSAS · SUBMISSION GATEWAY</div>
      </div>
...
// ── Watermark + security ──────────────────────────────
function Watermark() {
  const text = `KANSAS · CONFIDENTIAL · PATENT PENDING · U.S. 64/102,709 · ${new Date().toLocaleDateString()}`;
  return (
...
export default function GatewayApp() {
  ...
  if (!FLAGS.submission_gateway_enabled || !FLAGS.kansas_submission_gateway_enabled) {
    return <div style={{ padding: 40, color: T.charcoal }}>Submission Gateway is disabled.</div>;
  }

  const [selectedCaseIdx, setSelectedCaseIdx] = useState(0);
  const c = ALL_KS_CASES[selectedCaseIdx];
  const layers = [
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden", flexDirection: "column" }}>
        ...
        <div style={{ flex: 1, overflow: "auto", background: T.canvas }}>
          ...
          <div style={{ position: "sticky", top: 0, zIndex: 100, background: T.white, borderBottom: `1px solid ${T.border}`, padding: "10px 20px", display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <span style={{ fontSize: 11, fontWeight: 700, color: T.primary, background: T.light, border: `1px solid ${T.primary}33`, borderRadius: 999, padding: "3px 10px" }}>KanCare / KDHE</span>
